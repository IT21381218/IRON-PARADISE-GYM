...
const About = () => {
  ...
  const y1 = useTransform(scrollY, [0, 800], [0, -150]); // Background moves slower
  const y2 = useTransform(scrollY, [0, 800], [0, -50]); // Content moves slightly
  ...
  return (
    <div className="pt-20">
      ...
      <section className="relative py-12 sm:py-16 md:py-20 lg:py-24 overflow-hidden">
        <motion.div className="absolute inset-0" style={{ y: y1 }}>
          <img src={heroImg} alt="About Iron Paradise" className="w-full h-full object-cover" />
          <div className="absolute inset-0 bg-background/85" />
        </motion.div>

        {/* Floating Elements */}
        <motion.div
          className="absolute top-16 right-16 w-24 h-24 bg-primary/10 rounded-full blur-2xl"
          style={{ y: useTransform(scrollY, [0, 800], [0, 30]) }}
        />
        <motion.div
          className="absolute bottom-20 left-20 w-18 h-18 bg-primary/15 rounded-full blur-xl"
          style={{ y: useTransform(scrollY, [0, 800], [0, -20]) }}
        />

        <motion.div className="relative z-10 container mx-auto text-center px-4" style={{ y: y2 }}>
          <h1 className="font-heading text-5xl md:text-7xl font-bold text-foreground mb-4">
            About <span className="text-primary">Iron Paradise</span>
          </h1>
    ...
    <section className="section-padding">
      <div className="container mx-auto grid md:grid-cols-2 gap-12 items-center">
        <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
          ...
          <p className="text-muted-foreground leading-relaxed mb-4">Founded in 2015, Iron Paradise began as a small garage gym with a big dream. Today, we operate a premium facility in Homagama, serving over 5,000 active members.</p>
          <p className="text-muted-foreground leading-relaxed">Our founders—former competitive athletes—understood that great results require great environments. Every piece of equipment, every program, and every trainer at Iron Paradise is selected to deliver excellence.</p>
        </motion.div>
    ...
    <section className="section-padding bg-card text-center">
      <div className="container mx-auto">
        <h2 className="font-heading text-3xl md:text-5xl font-bold text-foreground mb-4">Join the <span className="text-primary">Iron Paradise</span> Family</h2>
